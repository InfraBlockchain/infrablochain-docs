---
title: 거래 형식
description: Substrate에서 서명된 및 서명되지 않은 거래의 형식을 설명합니다.
keywords:
---

이 문서에서는 Substrate에서 서명된 및 서명되지 않은 거래의 데이터 구조에 대해 자세히 설명합니다.
...
우선 순위는 디스패치 클래스와 보내는 사람이 지불할 의도로 하는 단위 무게당 팁 또는 길이당 팁(더 제한적인 값)에 따라 달라집니다.
