...
| 매개변수      | 설명                                                                                      |
| :------------- | :---------------------------------------------------------------------------------------- |
| `fees`         | 거래 수수료를 지불하기 위해 보유 레지스터에서 제거할 자산을 지정합니다.                     |
...
환불된 무게 레지스터를 초과 무게 레지스터의 값으로 증가시킵니다.
이 명령은 이전에 BuyExecution 명령을 사용하여 지불한 수수료를 환불된 무게 레지스터에 추가된 금액과 일치하도록 홀딩 레지스터로 이동시킵니다.
...
이 명령의 예상 무게에는 `appendix` 코드의 예상 무게가 포함되어야 합니다.
실행 시에 부록의 예상 무게가 변경되기 전에 초과 무게 레지스터의 예상 무게가 증가해야 합니다.
...
이 명령의 예상 무게에는 `error_handler` 코드의 예상 무게가 포함되어야 합니다.
실행 시에 초과 무게 레지스터의 예상 무게가 변경되기 전에 오류 처리기의 예상 무게가 증가해야 합니다.
...
| `max_weight`  | 인코딩된 호출을 디스패치하는 동안 사용할 최대 무게를 지정합니다. 지정한 무게보다 더 많은 무게가 필요한 경우 실행이 중지되고 오류가 반환됩니다. 지정한 무게보다 적은 무게가 필요한 경우 실행 시에 초과 무게 레지스터에 차이가 추가될 수 있습니다.                                                                                                                  |
...
| Surplus weight    | 이전에 계산된 무게의 과대 추정을 저장합니다.                                                   |
| Refunded weight   | 환불된 무게의 일부를 저장합니다.                                                               |
