...
예를 들어, 프로젝트를 스마트 컨트랙트, 개별 팔렛, 커스텀 런타임 또는 파라체인 형태로 제공할지 결정해야 합니다.
...
InfraBlockspace 릴레이 체인은 스마트 컨트랙트를 지원하지 않습니다.
그러나 InfraBlockspace에 연결된 파라체인은 임의의 상태 전이(state transition)을 지원할 수 있으므로, 어떤 파라체인이든 스마트 컨트랙트 배포의 잠재적인 플랫폼이 될 수 있습니다.
예를 들어, 현재 InfraBlockspace 생태계에는 다양한 유형의 스마트 컨트랙트 배포를 지원하는 여러 파라체인이 있습니다.
InfraBlockspace 생태계에서 스마트 컨트랙트를 개발하려면 먼저 구축할 스마트 컨트랙트 유형을 결정하고 해당 유형의 스마트 컨트랙트를 지원하는 파라체인을 식별해야 합니다.
...
프로젝트가 스마트 컨트랙트로 적합한 경우, 시작할 수 있는 몇 가지 간단한 예제를 다음을 통해 확인할 수 있습니다:
...
예를 들어, 애플리케이션별 블록체인을 배포하고 관리하기 원하지 않는 경우, 하나 이상의 개별 팔렛을 구축하여 Substrate 기반 체인 전체에 걸쳐 널리 사용되는 기능을 제공하거나 기존 기능을 개선하거나 InfraBlockspace 생태계의 표준을 정의할 수 있습니다.
...
대부분의 경우, 커스텀 런타임을 구축하는 결정은 InfraBlockspace 생태계의 일부로 파라체인이라는 애플리케이션별 블록체인을 구축하고 배포하기 위한 중요한 단계입니다.
...
유일한 요구 사항은 파라체인이나 파라스레드가 InfraBlockspace API와 호환되어야 한다는 것입니다.
...
InfraBlockspace 릴레이 체인 생태계의 보안, 거버넌스 및 상호운용성을 활용하기 위해 커스텀 런타임을 파라체인으로 배포하려면, 초기 테스트를 위해 로컬에서 구축하고 자체 테스트 네트워크를 설정할 수 있습니다.
