...
description: 개인 블록체인 네트워크를 위한 계정 키와 사용자 정의 체인 사양 생성하기
keywords:
  - 기업
  - 개인
  - 허가
...
블록체인에 사용할 키를 생성한 후, 해당 키 쌍을 사용하여 사용자 정의 **체인 사양**을 생성하고, 이 사용자 정의 체인 사양을 **검증자**라고 불리는 신뢰할 수 있는 네트워크 참여자와 공유할 수 있도록 준비합니다.

다른 사람들이 개인 블록체인 네트워크에 참여할 수 있도록 하려면, 그들이 고유한 키를 생성하도록 해야 합니다.
네트워크 참여자의 키를 수집한 후, `local` 체인 사양을 대체할 사용자 정의 체인 사양을 생성할 수 있습니다.

이 튜토리얼에서는 두 노드 네트워크를 보여주기 위해 `local` 체인 사양을 수정한 사용자 정의 체인 사양을 생성합니다.
만약 필요한 키를 가지고 있다면, 네트워크에 더 많은 노드를 추가하는 방법은 동일한 단계를 따르면 됩니다.

### 로컬 체인 사양 수정하기

완전히 새로운 체인 사양을 작성하는 대신, 미리 정의된 `local` 체인 사양을 수정할 수 있습니다.

로컬 사양을 기반으로 새로운 체인 사양을 생성하려면 다음 단계를 따르세요:

1. 컴퓨터에서 터미널 쉘을 엽니다.

1. Substrate 노드 템플릿을 컴파일한 루트 디렉토리로 이동합니다.

1. 다음 명령을 실행하여 `local` 체인 사양을 `customSpec.json`이라는 파일로 내보냅니다:
...
1. `name` 필드를 수정하여 이 체인 사양을 사용자 정의 체인 사양으로 식별합니다.
...
앞에서 보았듯이, `aura` 및 `grandpa` 섹션을 수정하여 체인 사양에 권한 주소를 추가하거나 변경할 수 있습니다.
...
검증자 정보가 포함된 체인 사양을 준비한 후, 사용하기 전에 원시 사양 형식으로 변환해야 합니다.
원시 체인 사양에는 변환되지 않은 사양과 동일한 정보가 포함되어 있습니다.
그러나 원시 체인 사양에는 노드가 로컬 저장소에서 데이터를 참조하는 데 사용하는 인코딩된 저장소 키도 포함되어 있습니다.
원시 체인 사양을 배포하면 각 노드가 올바른 저장소 키를 사용하여 데이터를 저장하도록 보장할 수 있습니다.

체인 사양을 원시 형식으로 변환하려면 다음을 수행하세요:

1. 컴퓨터에서 터미널 쉘을 엽니다.

1. Substrate 노드 템플릿을 컴파일한 루트 디렉토리로 이동합니다.

1. 다음 명령을 실행하여 `customSpec.json` 체인 사양을 `customSpecRaw.json` 파일로 원시 형식으로 변환합니다:
...
사용자 정의 체인 사양을 모든 네트워크 참여자에게 배포한 후, 자체 개인 블록체인을 시작할 준비가 되었습니다.
...
- 사용자 정의 체인 사양을 업데이트하여 블록 생성 (`aura`) 및 블록 최종화 (`grandpa`)를 위한 키를 포함했는지 확인하세요.

- 사용자 정의 체인 사양을 원시 형식으로 변환하고, 원시 체인 사양을 개인 네트워크에 참여하는 노드에게 배포했는지 확인하세요.
...
1. 다음 명령을 실행하여 커스텀 체인 사양을 사용하여 첫 번째 노드를 시작합니다:
...
- `--chain` 명령줄 옵션은 사용자 정의 체인 사양을 지정합니다.
...
- 출력에는 사용자 정의 체인 사양이 사용되고 있는 것이 나타납니다. 이는 `--chain` 명령줄 옵션을 사용하여 지정한 사용자 정의 체인 사양입니다.
...
1. 다음 명령을 실행하여 두 번째 블록체인 노드를 시작합니다:
   ...
   `--chain` 명령줄 옵션은 사용할 체인 사양 파일을 지정합니다.
...
1. 다음 명령을 실행하여 `grandpa` 비밀 키를 키스토어에 추가합니다. `key` 하위 명령을 사용하여 생성된 비밀 키를 사용합니다.
   ...
   이 네트워크는 체인 사양에 두 개의 검증자가 구성되어 있으므로, 두 번째 노드가 키를 추가한 후에야 블록 최종화가 시작될 수 있습니다.
