...
- 상향식 메시지 전달 (UMP)은 패러체인이 중계 체인으로 메시지를 전달할 수 있도록 합니다.
- 하향식 메시지 전달 (DMP)은 중계 체인이 패러체인으로 메시지를 전달할 수 있도록 합니다.
- 크로스 컨센서스 메시지 전달 (XCMP)은 패러체인이 서로 메시지를 보낼 수 있도록 합니다.

수평 중계 라우팅된 메시지 전달 (HRMP)은 크로스 컨센서스 메시지 전달 (XCMP)의 중간 버전입니다.
이 중간 솔루션인 XCMP-Lite는 XCMP와 동일한 기능을 제공하지만, 전달된 메시지를 모두 중계 체인 저장소에 저장합니다.
...
- 로컬 패러체인에서 두 패러체인 모두가 Sudo 팔레트를 사용할 수 있도록 설정했습니다.
...
패러체인이 다른 패러체인과 메시지를 교환하려면, 실행할 XCM 명령에 대한 비용을 지불하기 위해 중계 체인에 있는 계정에 자산이 있어야 합니다.

중계 체인에 주권 계정 주소를 추가하려면 다음을 수행하세요:

1. [Polkadot/Substrate Portal](https://polkadot.js.org/apps)을 열고 중계 체인 엔드포인트에 연결합니다.

2. 패러체인 [주권 계정 주소](https://substrate.stackexchange.com/questions/1200/how-to-calculate-sovereignaccount-for-parachain)를 계산하여 중계 체인에서 사용할 수 있도록 합니다.
   ...
   또한 중계 체인에서 패러체인에 사용되는 계정 주소는 다른 패러체인에서 사용되는 주소와 다릅니다.
...
2. 필요한 경우 현재 중계 체인의 구성 설정을 확인합니다.

   현재 중계 체인의 구성 설정을 확인하려면:
...
4. **hrmp**를 선택한 다음 **hrmpInitOpenChannel(recipient, proposedMaxCapacity, proposedMaxMessageSize)**를 선택하여 새 채널을 열기 위한 요청을 초기화합니다.
   ...
   **proposedMaxCapacity** 및 **proposedMaxMessageSize**에 설정하는 값은 중계 체인의 `hrmpChannelMaxCapacity` 및 `hrmpChannelMaxMessageSize` 매개변수에 정의된 값보다 크지 않아야 합니다.
...
인코딩된 호출을 보유하고 있으므로, 중계 체인을 통해 패러체인 A에서 패러체인 B로 채널을 열기 위한 요청을 구성할 수 있습니다.
...
4. 메시지를 전달할 상대 위치를 나타내기 위해 대상 매개변수를 지정합니다.
   ...
   이 예에서는 패러체인 1000의 상위 체인이 중계 체인이며, 상위 체인의 컨텍스트에서 Here 내부 설정은 중계 체인이 XCM을 실행할 것임을 의미합니다.
...
트랜잭션을 제출한 후, 중계 체인에서 메시지가 수신되고 실행되었는지 확인해야 합니다.

요청을 확인하려면 다음을 수행하세요:

1. 두 개의 별도 브라우저 탭 또는 창을 열고, 하나의 인스턴스는 중계 체인에 연결하고 다른 인스턴스는 패러체인 엔드포인트에 연결합니다.
...
4. 중계 체인에 연결된 브라우저 인스턴스에서 **네트워크**를 클릭한 다음 **탐색기**를 선택합니다.

3. 최근 이벤트 목록을 확인하고 **hrmp.OpenChannelRequested** 및 **ump.ExecutedUpward** 이벤트가 표시되는지 확인합니다.

   ![중계 체인에서 XCM 명령을 위한 이벤트](/media/images/docs/tutorials/parachains/hrmp-relay-chain-request-complete.png)
...
첫 번째 단계는 중계 체인에서 인코딩된 호출을 준비하는 것입니다.

인코딩된 호출을 준비하려면 다음을 수행하세요:

1. [Polkadot/Substrate Portal](https://polkadot.js.org/apps)을 열고 중계 체인에 연결합니다.
...
트랜잭션을 제출한 후, 중계 체인에서 메시지가 수신되고 실행되었는지 확인할 수 있습니다.
...
5. 최근 이벤트 목록을 확인하고 **hrmp.OpenChannelAccepted** 및 **ump.ExecutedUpward** 이벤트가 표시되는지 확인합니다.
   ...
   ![중계 체인에서 XCM 명령을 위한 이벤트](/media/images/docs/tutorials/parachains/hrmp-relay-chain-request-accepted.png)

   다음 에포크 시작 후에는 **hrmpChannels**를 쿼리하여 패러체인 A (1000)에서 패러체인 B (1001)로 채널을 열었는지 확인할 수 있습니다.
...
패러체인 B에서 패러체인 A로 채널을 열면 두 패러체인은 서로에게 직접 메시지를 보낼 수 있거나 메시지를 중계 체인을 통해 라우팅할 수 있습니다.
