...
이 팔레트를 더 유용하고 현실적인 간단한 collectibles 거래 애플리케이션의 기반으로 만들기 위해 몇 가지 기본 기능을 추가하여 다음 작업을 수행할 수 있도록 합시다:

- 계정 간 collectibles 이전하기.
- collectibles의 가격 설정하기.
- collectibles 구매하기.

## 이전하기

사용자가 collectibles을 한 계정에서 다른 계정으로 이전할 수 있도록 하려면 공개적으로 호출 가능한 함수를 생성해야 합니다.
이 워크샵에서는 공개 함수인 `transfer`가 내부 함수인 `do_transfer`를 호출하여 체크를 수행하고 변경 사항을 스토리지에 기록하도록 할 것입니다.
내부 함수에는 다음과 같은 체크가 포함되어 collectibles을 이전할 수 있는지 여부를 결정합니다:

- 이전 가능한 collectibles은 존재해야 합니다.
- collectibles은 현재 소유자에게 이전할 수 없습니다.
- collectibles은 이미 최대 허용 collectibles 수를 가진 계정에게 이전할 수 없습니다.

collectibles이 존재하고 현재 소유자에게 이전되지 않는 경우, `do_transfer` 내부 함수는 `OwnerOfCollectibles` 스토리지 맵을 업데이트하여 새로운 소유자를 반영하고 기본 데이터베이스에 변경 사항을 기록합니다.
...
1. Error Enum에 `NoCollectible`, `NotOwner`, `TransferToSelf` 변형을 추가하세요.
	 ...
	 pub enum Error<T> {
		 ...
		 BoundsOverflow,
		 /// collectibles이 존재하지 않습니다.
		 NoCollectible,
		 /// 소유자가 아닙니다.
		 NotOwner,
		 /// 자신에게 collectibles을 이전하려고 합니다.
		 TransferToSelf,
	 }
   ```
...
2. 팔레트에 `TransferSucceeded` 이벤트를 추가하세요.
	 ...
	 pub enum Event<T: Config> {
		  /// 새로운 collectibles이 성공적으로 생성되었습니다.
			...
			/// collectibles이 성공적으로 이전되었습니다.
			TransferSucceeded { from: T::AccountId, to: T::AccountId, collectible: [u8; 16] },
	 }
	 ```

3. collectibles 이전을 가능하게 하는 내부 함수를 생성하세요.
   
	 ```rust
	 // 스토리지 업데이트하여 collectibles 이전하기
	 pub fn do_transfer(
...
1. 호출 가능한 함수를 생성하세요.
	 ...
	 /// collectibles을 보유한 어떤 계정이든 다른 계정으로 보낼 수 있습니다.
	 /// 이전은 collectibles의 가격을 재설정하여 판매하지 않음으로 표시합니다.
	 #[pallet::weight(0)]
	 pub fn transfer(
		  origin: OriginFor<T>,
			to: T::AccountId,
...
collectibles 소유자가 소유한 collectibles의 가격을 설정할 수 있도록 하기 위해 팔레트는 Collectible 데이터 구조의 가격 필드를 업데이트하고 이벤트를 발생시키는 함수를 제공해야 합니다.
...
- 호출자는 서명된 원본이어야 합니다.
- collectibles은 이미 존재해야 합니다.
- 호출자는 collectibles의 소유자여야 합니다.
...
1. `PriceSet` 이벤트를 팔레트에 추가하세요.
	 ...
	 pub enum Event<T: Config> {
		...
		CollectibleCreated { collectible: [u8; 16], owner: T::AccountId },
     	/// collectibles이 성공적으로 이전되었습니다.
     	TransferSucceeded { from: T::AccountId, to: T::AccountId, collectible: [u8; 16] },
		/// collectibles의 가격이 성공적으로 설정되었습니다.
...
2. 가격을 설정하는 호출 가능한 함수를 추가하세요.
    ...
    ```rust
    /// collectibles 가격을 업데이트하고 스토리지에 기록합니다.
    #[pallet::weight(0)]
    pub fn set_price(
        origin: OriginFor<T>,
        unique_id: [u8; 16],
        ...
        // collectibles이 존재하고 호출자가 소유자인지 확인합니다.
...
사용자가 collectibles을 구매할 수 있도록 하려면 호출 가능한 다른 함수인 `buy_collectible`를 노출해야 합니다. 이 함수는 이전 호출 가능한 함수와 마찬가지로 내부 함수를 사용하여 변경 사항을 스토리지에 기록하기 전에 체크를 수행합니다.

이 워크샵에서 내부 함수는 `do_buy_collectible`이며, collectibles 구매 시도가 성공할지 여부를 결정하는 데 대부분의 작업을 수행합니다.
예를 들어, `do_buy_collectible` 내부 함수는 다음을 확인합니다:

- 제안된 구매 가격이 collectibles 소유자가 설정한 가격보다 크거나 같은지 확인하고, 제안된 가격이 너무 낮으면 `BidPriceTooLow` 오류를 반환합니다.
- collectibles이 판매 중인지 확인하고, collectibles 가격이 `None`인 경우 `NotForSale` 오류를 반환합니다.
- 구매자 계정에 collectibles 가격을 지불할 수 있는 충분한 잔액이 있는지 확인합니다.
- 구매자 계정이 이미 너무 많은 collectibles을 소유하여 다른 collectibles을 받을 수 없는지 확인합니다.

모든 체크가 통과되면 `do_buy_collectible` 내부 함수는 계정 잔액을 업데이트하고 `Currency` 트레이트의 transfer 메서드를 사용하여 collectibles의 소유권을 이전합니다.

대부분의 작업이 내부 함수에서 수행되므로 공개된 `buy_collectible` 함수는 호출자의 계정을 확인하고 `do_buy_collectible` 함수를 호출하기만 하면 됩니다.

1. `BidPriceTooLow`와 `NotForSale`을 Error enum의 변형에 추가하세요.
...
1. 사용자가 collectibles을 구매할 때 호출되는 내부 함수를 생성하세요.
	 ...
	 // 이전이 성공적으로 이루어졌으므로 collectibles 소유자를 업데이트하고 가격을 `None`으로 재설정합니다.
	 collectible.owner = to.clone();
	 collectible.price = None;
	 // 업데이트를 스토리지에 기록합니다.
	 CollectibleMap::<T>::insert(&unique_id, collectible);
...
1. 사용자가 collectibles을 구매할 수 있는 호출 가능한 함수를 추가하세요.
	 ...
	 pub fn buy_collectible(
		 ...
		 // 판매자에서 구매자로 collectibles을 이전합니다.
