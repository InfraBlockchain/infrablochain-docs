...
이는 드물게 사용되는 사용 사례이지만, [build-only-wasm.sh](https://github.com/paritytech/polkadot-sdk/blob/master/substrate/.maintain/build-only-wasm.sh) 스크립트를 사용하여 `no_std` WebAssembly 바이너리 파일을 네이티브 런타임을 컴파일하지 않고 빌드할 수 있습니다.
