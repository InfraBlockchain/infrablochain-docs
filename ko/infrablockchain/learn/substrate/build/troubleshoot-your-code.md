...
- [의존성](https://developerexperience.io/articles/updating-the-dependencies). 정기적으로 의존성을 업데이트하여 코드가 새로운 릴리스와 너무 뒤떨어지지 않고 최신 상태를 유지할 수 있도록 하세요. 주기적으로 Rust 컴파일러와 툴체인을 업데이트하고, ***인프라블록체인(InfraBlockchain)*** 의 새로운 릴리스가 있는지 확인하세요.
